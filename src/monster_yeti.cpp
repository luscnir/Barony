...
void initYeti(Entity* my, Stat* myStats)
{
	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		if ( myStats != NULL )
		{
			...
			{
				strcpy(myStats->name, "The abominable snowman");
				for ( c = 0; c < 2; c++ )
				{
	...
	// torso
	Entity* entity = newEntity(912, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	...
	entity->focalx = limbs[YETI][1][0]; // 0
	entity->focaly = limbs[YETI][1][1]; // 0
	entity->focalz = limbs[YETI][1][2]; // 0
	entity->behavior = &actYetiLimb;
	entity->parent = my->getUID();
	...
	// right leg
	entity = newEntity(916, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[YETI][2][0]; // 1
	entity->focaly = limbs[YETI][2][1]; // 0
	entity->focalz = limbs[YETI][2][2]; // 2
	entity->behavior = &actYetiLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left leg
	entity = newEntity(915, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[YETI][3][0]; // 1
	entity->focaly = limbs[YETI][3][1]; // 0
	entity->focalz = limbs[YETI][3][2]; // 2
	entity->behavior = &actYetiLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// right arm
	entity = newEntity(914, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[YETI][4][0]; // -.25
	entity->focaly = limbs[YETI][4][1]; // 0
	entity->focalz = limbs[YETI][4][2]; // 3
	entity->behavior = &actYetiLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left arm
	entity = newEntity(913, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[YETI][5][0]; // -.25
	entity->focaly = limbs[YETI][5][1]; // 0
	entity->focalz = limbs[YETI][5][2]; // 3
	entity->behavior = &actYetiLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
...
void yetiDie(Entity* my)
{
	...
	playSoundEntity(my, 441 + rand() % 2, 128);
		...
		if ( bodypart == LIMB_HUMANOID_RIGHTLEG || bodypart == LIMB_HUMANOID_LEFTARM )
		{
			...
			if ( bodypart == LIMB_HUMANOID_RIGHTLEG || my->monsterAttack == 0 )
			{
				...
				if ( dist > 0.1 )
				{
					...
					{
						entity->pitch -= dist * YETIWALKSPEED;
