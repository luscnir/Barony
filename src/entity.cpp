	...
	else
	{
		switch ( myStats->type )
		{
			...
			case EYEBALL:
				...
				break;
			case FLESHLING:
				victim->setObituary(language[2174]);
				break;
			case ABOMINATION:
				victim->setObituary(language[2175]);
				break;
			case CHOLOROSH:
				victim->setObituary(language[2176]);
				break;
			case LICH_FALLEN:
...
bool Entity::isInvisible() const
{
	...
	// wearing invisibility mask
	if (entitystats->mask != NULL)
	{
		if (entitystats->mask->type == ABYSSAL_MASK )
		{
			return true;
		}
	}
...
bool Entity::isMobile()
{
	...
	if ( (entitystats->type == LICH_FIRE || entitystats->type == LICH_ICE || entitystats->type == LICH_FALLEN)
			...
			&& (!shapeshifted || (shapeshifted && myStats->type == CREATURE_IMP && itemCategory(myStats->weapon) == MAGICSTAFF)) )
		{
			...
			if ( itemCategory(myStats->weapon) == SPELLBOOK || itemCategory(myStats->weapon) == MAGICSTAFF )
			{
				if ( itemCategory(myStats->weapon) == MAGICSTAFF )
				{
					...
					if ( myStats->type == SHADOW || myStats->type == LICH_FIRE || myStats->type == LICH_ICE || myStats->type == LICH_FALLEN )
					{
				...
				if( hitsuccess )*/
				{
					...
					if ( weaponToBreak != nullptr && !shapeshifted )
					{
						...
						if ( !artifactWeapon )
						{
							...
							else if ( myStats->type == SHADOW || myStats->type == LICH_FIRE || myStats->type == LICH_ICE || myStats->type == LICH_FALLEN )
							{
								degradeWeapon = false; //certain monster's weapons don't degrade.
							}
...
void Entity::awardXP(Entity* src, bool share, bool root)
{
	...
	if ( player >= 0 )
	{
		...
		if ( currentlevel >= 46 && currentlevel <= 57 && srcStats->type == MINOTAUR )
		{
			for ( int c = 0; c < MAXPLAYERS; c++ )
			{
...
void Entity::handleHumanoidWeaponLimb(Entity* weaponLimb, Entity* weaponArmLimb)
{
	...
	if ( weaponLimb->flags[INVISIBLE] == false ) //TODO: isInvisible()?
	{
		...
		else if ( weaponLimb->sprite == items[CROSSBOW].index || weaponLimb->sprite == items[HEAVY_CROSSBOW].index || weaponLimb->sprite == items[ABYSSAL_CROSSBOW].index || weaponLimb->sprite == items[CANNON].index || weaponLimb->sprite == items[CANNON_BOULDER].index )
		{
	...
	else if ( !armBended )
	{
		...
		if ( weaponLimb->sprite == items[CROSSBOW].index || weaponLimb->sprite == items[HEAVY_CROSSBOW].index || weaponLimb->sprite == items[ABYSSAL_CROSSBOW].index || weaponLimb->sprite == items[CANNON].index || weaponLimb->sprite == items[CANNON_BOULDER].index )
		{
...
void Entity::handleEffectsClient()
{
	...
	//Amulet of Veagence //TODO: make particles work
	if (myStats->amulet)
	{
		if (myStats->amulet->type == ABYSSAL_AMULET)
		{
			if (myStats->HP <= 10)
			{
				spawnAmbientParticles(50, 1039, 10 + rand() % 30, 0.5, true);
			}
		}
	}
}
		...
		switch ( effect )
		{
			...
			case EFF_DISORIENTED:
				if ( myStats->type == LICH || myStats->type == DEVIL
					|| myStats->type == LICH_FIRE || myStats->type == LICH_ICE || myStats->type == LICH_FALLEN
...
void Entity::monsterAcquireAttackTarget(const Entity& target, Sint32 state, bool monsterWasHit)
{
	...
	if ( target.getStats() != nullptr )
	{
		...
		{
			if ( myStats->type != LICH_FIRE && myStats->type != LICH_ICE && myStats->type != LICH && myStats->type != DEVIL && myStats->type != LICH_FALLEN )
			{
...
void Entity::setHelmetLimbOffset(Entity* helm)
{
	...
	if ( helm->sprite == items[HAT_PHRYGIAN].index )
	{
		...
		|| helm->sprite == items[PUNISHER_HOOD].index || helm->sprite == items[HAT_HOOD_YELLOWGREEN].index)
	{
		switch ( monster )
		{
			case AUTOMATON:
