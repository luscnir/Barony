/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_burgguard.cpp
	Desc: implements all of the burg guard monster's code
...
void initBurgGuard(Entity* my, Stat* myStats)
{
	...
	if (multiplayer != CLIENT && !MONSTER_INIT)
	{
		if (myStats != nullptr)
		{
			...
			else
			{
				...
				for (c = 0; c < 3; c++)
				{
					Entity* entity = summonMonster(BURGGUARD, my->x, my->y);
					if (entity)
					{
			...
			if (myStats->cloak == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_CLOAK] == 1)
			{
				...
				case 7:
				case 8:
					myStats->cloak = newItem(CLOAK_YELLOWGREEN, WORN, 1 + rand() % 3, 1, rand(), false, nullptr);
					break;
				case 9:
					myStats->cloak = newItem(CLOAK_PROTECTION, WORN, 0, 1, rand(), false, nullptr);
					break;
				}
			}
			...
			if (myStats->helmet == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_HELM] == 1)
			{
				...
				case 3:
				case 4:
					myStats->helmet = newItem(HAT_HOOD_YELLOWGREEN, WORN, 1 + rand() % 3, 1, rand(), false, nullptr);
					break;
				case 5:
					myStats->helmet = newItem(HAT_WIZARD_SLIMY, WORN, -1 + rand() % 3, 1, rand(), false, nullptr);
					break;
				case 6:
				case 7:
				...
				case 8:
				case 9:
					myStats->helmet = newItem(IRON_HELM, WORN, -2 + rand() % 3, 1, rand(), false, nullptr);
					break;
				}
			}
			...
			if (myStats->breastplate == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_ARMOR] == 1)
			{
				...
				case 8:
					...
					break;
				case 9:
					myStats->breastplate = newItem(LEATHER_BREASTPIECE, DECREPIT, -2 + rand() % 3, 1, rand(), false, nullptr);
					break;
				}
			}
		}
	...
	entity->focalx = limbs[BURGGUARD][1][0]; // 0
	entity->focaly = limbs[BURGGUARD][1][1]; // 0
	entity->focalz = limbs[BURGGUARD][1][2]; // 0
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BURGGUARD][2][0]; // 0
	entity->focaly = limbs[BURGGUARD][2][1]; // 0
	entity->focalz = limbs[BURGGUARD][2][2]; // 2
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BURGGUARD][3][0]; // 0
	entity->focaly = limbs[BURGGUARD][3][1]; // 0
	entity->focalz = limbs[BURGGUARD][3][2]; // 2
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->focalz = limbs[BURGGUARD][4][2]; // 1.5
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->focalz = limbs[BURGGUARD][5][2]; // 1.5
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BURGGUARD][6][0]; // 1.5
	entity->focaly = limbs[BURGGUARD][6][1]; // 0
	entity->focalz = limbs[BURGGUARD][6][2]; // -.5
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	entity->pitch = .25;
	node = list_AddNodeLast(&my->children);
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BURGGUARD][7][0]; // 2
	entity->focaly = limbs[BURGGUARD][7][1]; // 0
	entity->focalz = limbs[BURGGUARD][7][2]; // 0
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BURGGUARD][8][0]; // 0
	entity->focaly = limbs[BURGGUARD][8][1]; // 0
	entity->focalz = limbs[BURGGUARD][8][2]; // 4
	entity->behavior = &actBurgGuardLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
...
void burgGuardMoveBodyparts(Entity * my, Stat * myStats, double dist)
{
	...
	for (bodypart = 0, node = my->children.first; node != nullptr; node = node->next, bodypart++)
	{
		...
		{
			my->humanoidAnimateWalk(entity, node, bodypart, BURGGUARDWALKSPEED, dist, 0.4);
		}
		...
		case LIMB_HUMANOID_TORSO:
			...
			my->setHumanoidLimbOffset(entity, BURGGUARD, LIMB_HUMANOID_TORSO);
			break;
			// right leg
		case LIMB_HUMANOID_RIGHTLEG:
			...
			my->setHumanoidLimbOffset(entity, BURGGUARD, LIMB_HUMANOID_RIGHTLEG);
			break;
			// left leg
...
bool Entity::burgGuardCanWieldItem(const Item& item) const
{
