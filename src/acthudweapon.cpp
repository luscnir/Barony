...
void actHudWeapon(Entity* my)
{
	...
	if (players[clientnum] && players[clientnum]->entity)
	{
		if (!levitating && !waterwalkingboots && !skillCapstoneUnlocked(clientnum, PRO_SWIMMING) )
		{
			...
			if ( swimmingtiles[map.tiles[y * MAPLAYERS + x * MAPLAYERS * map.height]] || lavatiles[map.tiles[y * MAPLAYERS + x * MAPLAYERS * map.height]] || poisontiles[map.tiles[y * MAPLAYERS + x * MAPLAYERS * map.height]] )
			{
				my->flags[INVISIBLE] = true;
				if (parent)
				{
	...
	else
	{
		...
		else
		{
			...
			if ( bowIsBeingDrawn && !(stats[clientnum]->weapon && stats[clientnum]->weapon->type == HEAVY_CROSSBOW) )
			{
	...
	else
	{
		...
		if ( stats[clientnum]->weapon && stats[clientnum]->weapon->type == HEAVY_CROSSBOW
			&& !stats[clientnum]->defending && !hideWeapon )
		{
#ifdef SOUND
#ifdef USE_OPENAL
			if ( bowDrawingSoundChannel )
				...
				if ( itemCategory(stats[clientnum]->weapon) == WEAPON || stats[clientnum]->weapon->type == TOOL_PICKAXE )
				{
					...
					else if ( rangedweapon )
					{
						...
						else
						{
							...
							{
								doAttack = true;
								bool heavyCrossbow = stats[clientnum]->weapon && stats[clientnum]->weapon->type == HEAVY_CROSSBOW;
	...
	else
	{
		...
		if (item)
		{
			...
			if (item->type == CROSSBOW || item->type == HEAVY_CROSSBOW || item->type == ABYSSAL_CROSSBOW || item->type == CANNON || item->type == CANNON_BOULDER )
			{
				my->x = 6 + HUDWEAPON_MOVEX;
...
void actHudShield(Entity* my)
{
	...
	if ( hudweapon && crossbow 
		&& hudweapon->skill[8] == CROSSBOW_ANIM_SWAPPED_WEAPON && rangedWeaponUseQuiverOnAttack(stats[clientnum]) )
	{
		// CROSSBOW_ANIM_SWAPPED_WEAPON requires the weapon to not be in the swapping animation to finish.
...
void actHudArrowModel(Entity* my)
{
	...
	if ( (!crossbow && !bow) || cast_animation.active || cast_animation.active_spellbook )
	{
		my->flags[INVISIBLE] = true;
		return;
	}
