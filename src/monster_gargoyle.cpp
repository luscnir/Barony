/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_gargoyle.cpp
	Desc: implements all of the gargoyle monster's code
...
void initGargoyle(Entity* my, Stat* myStats)
{
	...
	if (multiplayer != CLIENT && !MONSTER_INIT)
	{
		if (myStats != nullptr)
		{
			...
			// always give special spell to gargoyle, undroppable.
			...
			case 5:
			case 4:
				if (rand() % 20 == 0) // 5% drop levitation spellbook
				{
					newItem(static_cast<ItemType>(SPELLBOOK_LEVITATION), static_cast<Status>(1 + rand() % 4), -1 + rand() % 3, 1, rand(), false, &myStats->inventory);
				}
			case 3:
			case 2:
				if (rand() % 10 < 3) // 30% drop dig magicstaff
				{
					newItem(static_cast<ItemType>(MAGICSTAFF_DIGGING), static_cast<Status>(1 + rand() % 4), -1 + rand() % 3, 1, rand(), false, &myStats->inventory);
				}
			case 1:
	...
	entity->focalx = limbs[GARGOYLE][1][0]; // 0
	entity->focaly = limbs[GARGOYLE][1][1]; // 1
	entity->focalz = limbs[GARGOYLE][1][2]; // 0
	entity->behavior = &actGargoyleLimb;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[GARGOYLE][2][0]; // 0
	entity->focaly = limbs[GARGOYLE][2][1]; // 0
	entity->focalz = limbs[GARGOYLE][2][2]; // 2
	entity->behavior = &actGargoyleLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[GARGOYLE][3][0]; // 0
	entity->focaly = limbs[GARGOYLE][3][1]; // 0
	entity->focalz = limbs[GARGOYLE][3][2]; // 2
	entity->behavior = &actGargoyleLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[GARGOYLE][4][0]; // 0
	entity->focaly = limbs[GARGOYLE][4][1]; // 0
	entity->focalz = limbs[GARGOYLE][4][2]; // 3
	entity->behavior = &actGargoyleLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[GARGOYLE][5][0]; // 0
	entity->focaly = limbs[GARGOYLE][5][1]; // 0
	entity->focalz = limbs[GARGOYLE][5][2]; // 3
	entity->behavior = &actGargoyleLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[GARGOYLE][6][0]; // 0
	entity->focaly = limbs[GARGOYLE][6][1]; // 4
	entity->focalz = limbs[GARGOYLE][6][2]; // 0
	entity->behavior = &actGargoyleLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[GARGOYLE][7][0]; // 0
	entity->focaly = limbs[GARGOYLE][7][1]; // -4
	entity->focalz = limbs[GARGOYLE][7][2]; // 0
	entity->behavior = &actGargoyleLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	for (bodypart = 0, node = my->children.first; node != nullptr; node = node->next, bodypart++)
	{
		...
		if (bodypart == 3 || bodypart == 6)
		{
			...
			if (bodypart == 3 || !MONSTER_ATTACK)
			{
				...
				if (!leftbody->skill[0])
				{
					entity->pitch -= GARGOYLEWALKSPEED * limbSpeedMultiplier;
			...
			else
			{
				...
				else if (MONSTER_ATTACK == MONSTER_POSE_MELEE_WINDUP3)
				{
					if (MONSTER_ATTACKTIME == 0)
					{
						...
						entity->pitch = 0;
						entity->roll = 0;
						playSoundEntityLocal(my, 416, 128);
					...
					case 2:
						...
						if (limbAnimateToLimit(entity, ANIMATE_PITCH, -0.4, 5 * PI / 4, false, 0) == 1)
						{
							...
							if (multiplayer != CLIENT)
							{
								...
								my->attack(MONSTER_POSE_GARGOYLE_DOUBLEATTACK, 0, nullptr);
							}
						}
						break;
					default:
				...
				// double attack
				else if (MONSTER_ATTACK == MONSTER_POSE_GARGOYLE_DOUBLEATTACK)
				{
					switch (entity->skill[0])
					{
					case 2:
				...
				else if (MONSTER_ATTACK == MONSTER_POSE_MAGIC_WINDUP2)
				{
					if (MONSTER_ATTACKTIME == 0)
					{
						...
						playSoundEntityLocal(my, 417, 128);
						createParticleDot(my);
						if (multiplayer != CLIENT)
						{
							my->monsterAnimationLimbOvershoot = ANIMATE_OVERSHOOT_TO_SETPOINT;
